...
import {workflowApp}  from "../langraph/workflow.js";
...
export const generateWebsiteByAgent = async (req, res) => {
  try {
    const { prompt } = req.body;
    if (!prompt) return res.status(400).json({ message: "Prompt is required" });
    ...
    const website = await Website.create({
      user: user._id,
      title: prompt.slice(0, 50),
      latestCode: result.finalCode,
      conversation: [
        { role: "user", content: prompt },
        { role: "ai", content: result.message || "Website generated successfully" },
      ],
    });
